...
describe("addItem", () => {
  ...
  test("should throw 404 when cart does not exist", async () => {
    ...
    await expect(cartService.addItem("invalid-cart", product.id, 1)).rejects.toMatchObject({
      statusCode: 404,
  ...
  test("should throw 404 when product does not exist", async () => {
    ...
    await expect(cartService.addItem(cartId, "invalid-product", 1)).rejects.toMatchObject({
      statusCode: 404,
