...
export function AgeVerificationModal({ isOpen, onConfirmAdult, onConfirmMinor }) {
  const { language } = useTranslation();
  const [step, setStep] = useState("ask");

  if (!isOpen) return null;

  if (step === "confirmMinor") {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-md">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 p-8 text-center">
          <h2 className="text-2xl font-bold text-slate-900 mb-3">
            {language === "en" ? "Are you sure?" : "¿Estás seguro?"}
          </h2>
          <p className="text-slate-600 mb-6 leading-relaxed">
            {language === "en"
              ? "If you confirm, the Adult Toys section will be hidden on this device. You'll still be able to browse the rest of the store."
              : "Si confirmas, la sección de Juguetes Sexuales quedará oculta en este dispositivo. Podrás seguir navegando el resto de la tienda."}
          </p>
          <div className="flex flex-col gap-3">
            <button onClick={onConfirmMinor} className="w-full bg-slate-700 hover:bg-slate-800 text-white font-bold py-3 px-6 rounded-lg">
              {language === "en" ? "Yes, I'm under 18" : "Sí, soy menor de 18 años"}
            </button>
            <button onClick={() => setStep("ask")} className="w-full text-slate-500 hover:text-slate-700 font-medium py-2">
              {language === "en" ? "No, that was a mistake — I'm 18+" : "No, fue un error — soy mayor de edad"}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-md">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 p-8 text-center relative overflow-hidden">
        ...
        {/* Reject link */}
        <button
          onClick={() => setStep("confirmMinor")}
          className="w-full text-slate-400 hover:text-slate-600 mt-3 text-sm underline"
        >
          {language === "en" ? "I'm under 18" : "Soy menor de 18 años"}
        </button>
