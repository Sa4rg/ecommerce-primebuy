...
export function FaqPage() {
  ...
  const supportHref = buildWhatsAppLink({
    ...
    message: t("support.common.whatsappMessage"),
  });
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      <nav className="flex items-center gap-2 text-sm text-pb-text-secondary mb-8">
        ...
        <span className="text-pb-text font-medium">{t("support.faq.title")}</span>
      </nav>

      <div className="rounded-2xl border border-pb-border bg-white shadow-sm p-6 md:p-8">
        <h1 className="text-3xl font-extrabold text-pb-text">{t("support.faq.title")}</h1>
        <p className="mt-2 text-sm text-pb-text-secondary">
          {t("support.common.updatedAt", { date: formatToday(language) })}
        </p>

        <p className="mt-6 text-pb-text-secondary">{t("support.faq.intro")}</p>

        <div className="mt-8 space-y-6">
          <Section title={t("support.faq.sections.paymentsTitle")} text={t("support.faq.sections.paymentsText")} />
          <Section title={t("support.faq.sections.proofTitle")} text={t("support.faq.sections.proofText")} />
          <Section title={t("support.faq.sections.shippingTitle")} text={t("support.faq.sections.shippingText")} />
          <Section title={t("support.faq.sections.stockTitle")} text={t("support.faq.sections.stockText")} />
          <Section title={t("support.faq.sections.supportTitle")} text={t("support.faq.sections.supportText")} />
        </div>
