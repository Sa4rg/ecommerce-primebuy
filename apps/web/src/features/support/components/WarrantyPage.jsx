  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      <nav className="flex items-center gap-2 text-sm text-slate-400 mb-8">
        ...
        <span className="text-slate-100 font-medium">{t("support.warranty.title")}</span>
      </nav>

      <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 md:p-8">
        <h1 className="text-3xl font-extrabold text-slate-100">{t("support.warranty.title")}</h1>
        <p className="mt-2 text-sm text-slate-400">
          {t("support.common.updatedAt", { date: formatToday(language) })}
        </p>

        <p className="mt-6 text-slate-300">{t("support.warranty.intro")}</p>
        <p className="mt-3 text-sm text-slate-400">{t("support.warranty.scope")}</p>
