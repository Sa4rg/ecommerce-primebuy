...
export function WarrantyPage() {
  ...
  const supportHref = buildWhatsAppLink({
    ...
    message: t("support.common.whatsappMessageWarranty"),
  });
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      <nav className="flex items-center gap-2 text-sm text-pb-text-secondary mb-8">
        ...
        <span className="text-pb-text font-medium">{t("support.warranty.title")}</span>
      </nav>

      <div className="rounded-2xl border border-pb-border bg-white shadow-sm p-6 md:p-8">
        <h1 className="text-3xl font-extrabold text-pb-text">{t("support.warranty.title")}</h1>
        <p className="mt-2 text-sm text-pb-text-secondary">
          {t("support.common.updatedAt", { date: formatToday(language) })}
        </p>

        <p className="mt-6 text-pb-text-secondary">{t("support.warranty.intro")}</p>
        <p className="mt-3 text-sm text-pb-text-secondary">{t("support.warranty.scope")}</p>

        <div className="mt-8 space-y-6">
          <Section title={t("support.warranty.sections.coverageTitle")} text={t("support.warranty.sections.coverageText")} />
          <Section title={t("support.warranty.sections.exclusionsTitle")} text={t("support.warranty.sections.exclusionsText")} />
          <Section title={t("support.warranty.sections.processTitle")} text={t("support.warranty.sections.processText")} />
        </div>
